 ...
 * which resulted in an HTTP 500 (Internal Server Error).
 *
 * @author Alexander Hanschke
 * @since 1.0.0
 * @see DelegatingErrorHandler
 */
class InternalServerErrorHandler extends DefaultResponseErrorHandler {

    @Override
    public void handleError(ClientHttpResponse response) throws IOException {
        ErrorResponseWrapper wrapper = new ErrorResponseWrapper(response);
        throw new InternalServerErrorException("wunderlist", wrapper.getMessage());
    }
}
